...
#'
#' When use_iso_remov = TRUE, the inequality constraint integral(u_r dt) <=
#' iso_remov_target is activated and each scenario is solved via
#' optimal_control_isoperimetric() rather than optimal_control_shooting().
#' Scenarios where the unconstrained CDR is already below the target are
#' returned unchanged (constraint non-binding). The comparison summary gains
#' three additional columns: iso_remov_final (achieved integral of u_r),
#' iso_remov_gap (deviation from the target, negative when non-binding), and
#' iso_constraint_binding (logical, TRUE when the constraint actively limits CDR).
#'
...
#' @param use_iso_remov Logical: activate isoperimetric constraint on u_r (default: FALSE).
#'   When TRUE, optimal_control_isoperimetric() is used instead of
#'   optimal_control_shooting().
#' @param iso_remov_target Target value for integral of u_r, i.e. total CDR = B.
#'   Required when use_iso_remov = TRUE (default: NULL).
#' @param lambda_iso_bounds Two-element vector for the outer secant search bracket.
#'   The upper bound is always overridden to 0 internally (KKT condition:
#'   lambda_iso_remov <= 0). Only lambda_iso_bounds[1] is meaningful and must be
#'   negative enough to bracket the binding solution. Passed to
#'   optimal_control_isoperimetric() (default: c(-1, 0)).
...
#'       When use_iso_remov = TRUE also includes iso_remov_final, iso_remov_gap,
#'       and iso_constraint_binding.
#'   - year_first_1p5C: Named vector of years when temperature first reaches 1.5°C
#'   - year_peak_temp: Named vector of years when peak temperature occurs
#'   - year_mitig_capped: Named vector of years when mitigation is first capped
#'   - failed_scenarios: Named list of error messages for scenarios that failed
#'   - run_info: List of metadata about the comparison analysis
#'
#' @examples
#' # Standard comparison across 5 SSP scenarios
...
#' )
#'
#' # With isoperimetric constraint on u_r
#' comparison <- run_scenario_comparison(
#'   parameter_df = my_params,
#'   emissions_df = all_emissions_data,
#'   economic_df = all_economic_data,
#'   scenarios = c("SSP2-Baseline", "SSP3-Baseline"),
#'   use_iso_remov = TRUE,
#'   iso_remov_target = 200,
#'   lambda_iso_bounds = c(-10, 10)
#' )
...
run_scenario_comparison <- function(parameter_df,
                                    ...
                                    use_iso_remov = FALSE,          # Activate isoperimetric constraint on u_r
                                    iso_remov_target = NULL,        # Target value for integral of u_r
                                    lambda_iso_bounds = c(-1, 0),   # Initial bracket for outer secant search (upper always 0 by KKT)
  ...
  # Validate isoperimetric arguments
  if (use_iso_remov && is.null(iso_remov_target)) {
    stop("iso_remov_target must be supplied when use_iso_remov = TRUE")
  }
  ...
  if (verbose) {
    ...
    if (use_iso_remov) {
      cat("Isoperimetric constraint on u_r: ACTIVE\n")
      cat("  iso_remov_target:", iso_remov_target, "\n")
      cat("  lambda_iso_bounds: [", lambda_iso_bounds[1], ",", lambda_iso_bounds[2], "]\n")
    }
  ...
  run_single_scenario <- function(scenario_name) {
    tryCatch({
      ...
      # Branch: use isoperimetric outer loop or standard shooting method
      if (use_iso_remov) {
        result <- optimal_control_isoperimetric(
          parameter_df           = parameter_df,
          emissions_df           = emissions_df,
          economic_df            = economic_df,
          scenario               = scenario_name,
          iso_remov_target       = iso_remov_target,
          mitigation_delay_years = mitigation_delay_years,
          cdr_delay_years        = cdr_delay_years,
          lambda_iso_bounds      = lambda_iso_bounds,
  ...
  if (use_parallel && length(scenarios) > 1) {
    ...
    parallel_success <- tryCatch({
      ...
      parallel::clusterExport(cl, 
                              c("optimal_control_isoperimetric",
                                "optimal_control_shooting", 
  ...
  if (length(scenario_results) > 0) {
    ...
    for (i in seq_along(scenario_results)) {
      ...
      # Append isoperimetric diagnostics when constraint is active
      if (use_iso_remov) {
        summary_row$iso_remov_final       <- if (!is.null(run$iso_remov_state))
          run$iso_remov_state[run$n_years] else NA
        summary_row$iso_remov_gap         <- if (!is.null(run$iso_remov_gap))
          run$iso_remov_gap else NA
        summary_row$iso_constraint_binding <- if (!is.null(run$iso_constraint_binding))
          run$iso_constraint_binding else NA
      }
      
      summary_data[[i]] <- summary_row
    }
