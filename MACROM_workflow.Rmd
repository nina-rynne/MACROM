...
source(here::here("src", "optimal_control_core_V3.R"))
...
dashboard <- create_scenario_dashboard(
...
# ==============================================================================
# Analyse which parameters have the greatest influence on model outcomes
# 
# REQUIREMENTS: Must use LHS parameter sampling (run latin_hypercube_sampling_call)
# OUTPUTS: Saved automatically to output/ directory
# RUNTIME: Depends on number of LHS samples (e.g., 500 samples = 1-3 hours)

# Source required functions
source(here::here("src", "optimal_control_core_V3.R"))
source(here::here("src", "parameter_importance.R"))

# USER CHOICE: Select scenario for parameter importance analysis
scenario_for_importance <- "SSP2-Baseline"  # Typically use middle-of-road scenario

# Run parameter importance analysis
parameter_importance_results <- run_parameter_importance_analysis(
  parameter_df = parameter_df,           # Must have multiple rows from LHS
  emissions_df = emissions_df,
  economic_df = economic_df,
  scenario = scenario_for_importance,
  use_parallel = TRUE,
  save_results = TRUE,
  verbose = FALSE
)
...
# Display summary
if (!is.null(parameter_importance_results$importance_summary)) {
  print(parameter_importance_results$importance_summary)
}
```
...
# ==============================================================================
# Create visualisations showing parameter sensitivities and correlations
# OUTPUTS: Saved automatically to figs/ directory
...
# Check if results exist
if (!exists("parameter_importance_results")) {
  stop("Parameter importance results not found. Please run the parameter_importance_analysis chunk first.")
}

# Create parameter importance dashboard
importance_dashboard <- create_parameter_importance_dashboard(
  importance_results = parameter_importance_results,
  
  # USER CHOICE: Variables to analyse
...
dashboard <- create_delayed_deployment_dashboard(
  ...
  #          "temp_damage_cost", "total_cost", "mitig_cost", "remov_cost"
